...
<div class="container">
    <?php
    while ($row = $resultFilteredProducts->fetch_assoc()) {
      echo '<div class="product">';
      echo '<td><img width="150" src="' . $row["image_path"] . '"/></td>';
      echo '<h3 class="cat">' . $row['category'] . '</h3>';
      echo '<h3>' . $row['name'] . '</h3>';
      echo '<p>Price: $' . $row['price'] . '</p>';
      echo '<a href="produit.php?id=' . $row['id'] . '">Voir Produit</a>';
      echo '</div>';
    }
    ?>
  </div>
